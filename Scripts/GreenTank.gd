...
func _physics_process(delta):
	...
	if(okToShoot):
		...
		var firstRaycastResult = castBulletIgnoringTiles(getCannonTipPosition(), Vector2(1,0).rotated($Cannon.rotation))
		...
		if (firstRaycastResult && firstRaycastResult.normal.is_normalized() && firstRaycastResult.collider.is_in_group('walls')):
				...
				var secondRaycastResult = castBulletIgnoringTiles(newOrigin, newDirVector.bounce(firstRaycastResult.normal))
				if(secondRaycastResult && secondRaycastResult.normal.is_normalized() && secondRaycastResult.collider.is_in_group('walls')):
					...
					var thirdRaycastResult = castBulletIgnoringTiles(secondOrigin, secondDirVector.bounce(secondRaycastResult.normal))
